...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    if (rightChild<=last && compare(heap->data[rightChild],heap->data[leftChild])==-1) {
        if(compare(heap->data[rightChild],heap->data[position])==-1) {
            dySwap(heap,rightChild,position);
...
void buildHeap(DynamicArray* heap, compareFunction compare)
{
    ...
    for(int i=(heap->size/2)-1;i>=0;i--) {
        adjustHeap(heap,heap->size-1,i,compare);
    }
}
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
    ...
    int parent, current = heap->size;
    dyAdd(heap,value);

    while(current>0) {
        parent = (current-1)/2;
        if(compare(heap->data[current],heap->data[parent])==-1) {
