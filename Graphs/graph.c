 ...
 * Assignment 6
 * Name: Mathew McDade 
 * Date: Mon Nov 27 10:31:01 PST 2017
 */
...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    Deque *reachableVertices = dequeNew();
    Deque *searchStack = dequeNew();

    dequePushBack(searchStack,source);

    while(!dequeIsEmpty(searchStack) && destination->isVisited!=1) {
        Vertex *currentVertex = dequeBack(searchStack);
        dequePopBack(searchStack);

        if(!currentVertex->isVisited) {
            currentVertex->isVisited = 1;
            dequePushBack(reachableVertices,currentVertex);
            for(int i=0;i<currentVertex->numNeighbors;i++) {
                if(!(currentVertex->neighbors[i]->isVisited)) {
                    dequePushBack(searchStack,currentVertex->neighbors[i]);
                }
            }
        }
    }
    dequeDelete(reachableVertices);
    dequeDelete(searchStack);
    if(destination->isVisited) return 1;
    else return 0;
}
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    ...
    while(!dequeIsEmpty(searchStack) && destination->isVisited!=1) {
        Vertex *currentVertex = dequeFront(searchStack);
